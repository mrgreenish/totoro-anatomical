...
export type OrganStudy = 'brain' | 'heart' | 'eye' | 'lung';
...
export function defaultAnatomyState(): AnatomyState {
  return { mode: 'exterior', variant: 'male', status: 'idle', cut: { axis: 'x', position: .5, flipped: false },
    ...
    brainView: { available: false, status: 'closed' }, heartView: { available: false, status: 'closed' }, eyeView: { available: false, status: 'closed' }, lungView: { available: false, status: 'closed' } };
}
