...
## Detailed heart study

In Split or Exploded view, choose **Explore the heart**, or focus an exposed heart and use **Open heart view**. The isolated study has its own camera and lighting. Back or Escape restores the previous camera, cut, filters, variant and separation. The brain and heart share cancellation-safe study navigation and keep their own compressed asset caches.

The authored heart includes an asymmetric ventricular envelope and apex, atria and folded appendages, coronary grooves and tapering arterial/venous branches, epicardial fat, hollow great vessels, four internal chambers, trabeculae, chordae and eleven valve leaflets. Eleven tissue classes have separate physical materials. Geometry-aligned color, tangent normal, occlusion, roughness and optical thickness maps drive wet-film reflections and thickness-based subsurface transport. Deformed tissue normals follow the cardiac motion. These are real-time approximations; the model is human-inspired fictional anatomy, with synthesized textures rather than a clinical scan.

**Translucent** exposes the chamber interiors. **Blood flow** shows blood-filled lumens, density motion and advected tracers. **Blood cells** adds enlarged, biconcave erythrocytes. A shared 72 BPM cycle coordinates atrial filling, ventricular contraction, AV/semilunar valve opening and fluid velocity. Closed valves block particle crossings; continuous inlet, AV and outflow passages connect the chambers. Right-side blood travels toward the pulmonary arteries; left-side blood travels from the pulmonary veins through the mitral and aortic valves to the aorta. Chamber trajectories include recirculation and the vessel velocity profile varies across the lumen. The flow is an illustrative, kinematic simulation, not a Navier–Stokes solver or a source of clinical pressure/flow measurements. Cells are deliberately enlarged. Pause freezes the complete cycle, and reduced motion holds it still.

The detailed GLB is 1.62 MB / 292,901 triangles on desktop and 1.05 MB / 169,497 triangles on mobile. Opaque rendering draws only the eight external tissue batches. The full internal/cell view uses 25 draws, including a single instanced draw for 560 desktop or 240 mobile cells. Color is 4K on desktop and 2K on mobile; normal and packed surface maps remain 2K to control GPU memory. Effective lazy downloads are approximately 7.1 MB desktop and 4.3 MB mobile. GPU resources are released on exit, and invisible blood has no per-frame advection work. The current local WebGL measurements, viewport scope and screenshots are recorded in `artwork/anatomy/heart-detail/webgl-verification.json`; mobile measurements use viewport emulation, not phone hardware.

Rebuild with `blender --background --factory-startup --python scripts/build-heart-detail.py`, then `node scripts/optimize-heart-detail.mjs`. The editable Blender source, original texture reference, analytical maps, provenance and review renders are in `artwork/anatomy/heart-detail/`. Runtime assets are in `public/models/heart-detail/`. The optimizer preserves moving valve metadata and batches the other geometry. Normalized integer attributes must be promoted to floats before baking node transforms; otherwise aortic coordinates above one wrap and corrupt the rendered arch.

Run `node scripts/verify-heart-view.mjs`, `node scripts/verify-brain-view.mjs`, and `node scripts/verify-anatomy-runtime.mjs` for the shared navigation, actual assets, valve gating, transport, animation controls, cancellation/retry and lifecycle checks. `scripts/verify-heart-render.mjs` additionally uses a fresh local headless Chrome debugging session on port 9337 and the dev server on port 3002 to compile the real shaders and capture opaque, blood-only, cell, paused and mobile views.

Anatomical and cycle references include the University of Minnesota's [right ventricle atlas](https://www.vhlab.umn.edu/atlas/right-ventricle/index.shtml), [tricuspid valve atlas](https://www.vhlab.umn.edu/atlas/right-ventricle/tricuspid-valve/index.shtml) and [physiology tutorial](https://www.vhlab.umn.edu/atlas/physiology-tutorial/the-human-heart.shtml). These informed the authored approximation; no atlas images or meshes are embedded.

## Detailed eye study

In Split or Exploded, use **Look a little closer → Eye**. Brain, Heart, and Eye now share an icon-based selector and a persistent study switcher. Switching releases the previous organ's GPU resources; Back or Escape restores the original anatomy camera and settings. Loading includes cancellation and a retry action.

The eye has a photographed-looking hazel iris albedo with pupil-driven annular deformation, a refractive corneal dome, wet sclera with branching vessels and fine collagen relief, a biconvex deformable lens, ciliary muscle and processes, zonular fibers, vitreous, three ocular coats, retinal vessels, fovea, optic disc and optic nerve. The original parametric model is in `lib/eye-detail.ts`; physical transmission, immersed-lens refraction, thin-film reflections, tissue light transport and retinal membrane detail are in `lib/eye-materials.ts`. It is a human-inspired real-time approximation, not a clinical scan or a fully ray-traced gradient-index optical solver.

**Whole eye**, **Inside**, and **Rods & cones** expose the exterior, a sagittal cutaway, and magnified retinal tissue. Five child-friendly discoveries cover named eye parts, pupil response to light, near/far accommodation, overlapping cone sensitivities and rod response in dim light, and electrical messages to the brain. Disabling lens accommodation demonstrates a blurred near image. A diagram shows image inversion; the 3D rays stop at the retina. Retinal cell types can be hidden separately. Labels, pause and reduced-motion behavior are supported. The mobile layout keeps the model above the discoveries and uses fewer cells and smaller decoded textures.

The four lazy-loaded texture maps total under 500 KB. Desktop visible geometry is 31,552 triangles in 5 geometry draws for the whole eye, 64,608 in 20 for the cutaway with rays, and 58,016 in 6 for the retinal patch. These counts exclude extra physical-transmission passes; they are not frame-rate claims. The exterior settles to on-demand rendering; active teaching particles animate without React state updates. Shared instancing and shader membrane bands avoid thousands of separate retinal cell and ring meshes. GPU textures, geometries, materials and instance buffers are released on exit, while compressed texture blobs are cached for reopening.

Run `node scripts/build-eye-textures.mjs` to regenerate the deterministic tissue maps. The iris is an original built-in image generation, documented with its exact prompt in `artwork/anatomy/eye-detail/iris-provenance.md`. Run `node scripts/verify-eye-view.mjs` for optics, model budgets, controls, study switching, camera restoration, lazy caching, pause/reduced motion, retry/cancellation and resource disposal. Its Node integration results are in `artwork/anatomy/eye-detail/verification.json`; GPU rendering and physical-phone performance have not been measured for this addition.
