...
The hosted source snapshot includes the finished browser model and the deterministic Blender builder. Large `.blend` and uncompressed `.glb` authoring outputs are retained in the original local project and can be regenerated with the builder; they are omitted from the source upload.
